...
- 일반 파일과 ZIP 폴더 업로드
- 32MB 단위·파일 3개 × 파일당 3조각 병렬 업로드와 파일별 진행 상태 표시
- 업로드 취소와 중단된 세션 복구
...
대용량 파일은 브라우저에서 32MB 조각으로 나뉘며, 최대 3개 파일을 동시에 파일당 3조각씩 전송합니다. 서버는 세션 정보를 임시 디스크에 기록하므로 프로세스가 재시작돼도 이미 받은 조각을 다시 찾을 수 있습니다.
